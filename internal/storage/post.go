...
func (p *PostStorage) CreatePost(post models.Post) error {
	...
	_, err := p.db.Exec(query, post.Title, post.Description, post.Author, categoriesStr)
	if err != nil {
		return err
	}
	return nil
...
func (p *PostStorage) GetPostByID(id int) (models.Post, error) {
	query := `SELECT id, author, title, description, created_at, likes, dislikes, category FROM post WHERE id = $1;`
	row := p.db.QueryRow(query, id)
	var post models.Post
	var categoriesStr string
	if err := row.Scan(&post.Id, &post.Author, &post.Title, &post.Description, &post.CreateAt, &post.Likes, &post.Dislikes, &categoriesStr); err != nil {
