...
func (p *PostStorage) GetAllPostsByCategories(category string) ([]models.Post, error) {
	query := `
		SELECT id, title, description, author, likes, dislikes, category, created_at
		FROM post
	...
	for rows.Next() {
		...
		err := rows.Scan(&post.Id, &post.Title, &post.Description, &post.Author, &post.Likes, &post.Dislikes, &cats, &post.CreateAt)
		if err != nil {
			return nil, err
		}
